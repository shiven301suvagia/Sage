...
export class ConfirmationVault{
 constructor({ttlMs=120000,maxPending=32,clock=()=>Date.now(),idFactory=()=>randomUUID()}={}){this.ttlMs=Math.max(1,Number(ttlMs)||120000);this.maxPending=Math.max(1,Math.floor(Number(maxPending)||32));this.clock=clock;this.idFactory=idFactory;this.items=new Map();}
...
export class RecoveryBudget{
 constructor({maxAttempts=3,windowMs=30000,clock=()=>Date.now()}={}){this.maxAttempts=Math.max(1,Math.floor(Number(maxAttempts)||3));this.windowMs=Math.max(1,Number(windowMs)||30000);this.clock=clock;this.attempts=[];}
