...
export class ReminderStore{
 ...
 async load(){try{const parsed=JSON.parse(await fs.readFile(this.file,'utf8'));this.items=Array.isArray(parsed)?parsed.filter(isReminder):[];}catch(e){if(e?.code!=='ENOENT')await this.#recoverCorrupt('parse');this.items=[];}return this.snapshot();}
 ...
 async #recoverCorrupt(reason){const stamp=new Date().toISOString().replace(/[:.]/g,'-');const backup=`${this.file}.corrupt-${reason}-${stamp}`;try{await fs.rename(this.file,backup);}catch(e){if(e?.code!=='ENOENT')throw e;}await this.#save();}
}
