...
export class CharacterRuntime{#state;#listeners=new Set();constructor(initialState='dormant'){if(!SAGE_STATES.includes(initialState))throw new Error(`Unknown state: ${initialState}`);this.#state=initialState;}get state(){return this.#state;}canTransition(next){return SAGE_STATES.includes(next)&&ALLOWED.get(this.#state)?.has(next)===true;}transition(next){if(next===this.#state)return false;if(!this.canTransition(next))return false;const previous=this.#state;this.#state=next;for(const listener of this.#listeners)listener({previous,state:next});return true;}subscribe(listener){this.#listeners.add(listener);return()=>this.#listeners.delete(listener);}}
