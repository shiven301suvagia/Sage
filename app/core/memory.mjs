...
export class MemoryStore{
 ...
 async remove(id){const key=String(id??'');if(!key)return false;const before=this.#data.entries.length;this.#data.entries=this.#data.entries.filter(entry=>entry.id!==key);if(this.#data.entries.length===before)return false;await this.#persist();return true;}
