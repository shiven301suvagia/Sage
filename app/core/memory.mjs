...
function normalizeEntry(entry){return{id:String(entry.id),text:String(entry.text).slice(0,MAX_TEXT),createdAt:String(entry.createdAt),updatedAt:String(entry.updatedAt||entry.createdAt),...sanitizeMetadata(entry,{preserveCore:false})};}
function sanitizeMetadata(metadata,{preserveCore=true}={}){if(!metadata||typeof metadata!=='object'||Array.isArray(metadata))return{};const out={};for(const[key,value]of Object.entries(metadata).slice(0,MAX_METADATA_KEYS)){if(preserveCore&&['id','text','createdAt','updatedAt'].includes(key))continue;if(typeof value==='string')out[key]=value.slice(0,MAX_METADATA_VALUE);else if(typeof value==='boolean'||typeof value==='number')out[key]=value;else if(value===null)out[key]=null;}return out;}
