...
export class ProactiveEngine{
 ...
 #applyEmotion(emotion){const target=this.#emotionState(emotion);if(this.runtime?.state!==target){let transitioned=false;if(this.runtime?.canTransition?.(target))transitioned=this.runtime.transition(target);else if(this.runtime?.canTransition?.('idle')){this.runtime.transition('idle');if(this.runtime?.canTransition?.(target))transitioned=this.runtime.transition(target);}if(!transitioned)return null;}const state=this.runtime?.state||target;this.onStateChange({state,emotion});return state;}
