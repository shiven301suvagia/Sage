...
test('Phase 17 rebuild: runtime is OS neutral and uses injectable timing',async()=>{const source=await read('app/core/runtime.mjs');assert.doesNotMatch(source,/process\.platform|win32|darwin|linux/);let timer=null;let cleared=null;const runtime=new CharacterRuntime('idle',{sleepAfterMs:100,clock:(fn,ms)=>{timer={fn,ms};return timer;},clearClock:value=>{cleared=value;}});assert.equal(runtime.scheduleSleep(),true);const stale=timer;assert.equal(runtime.wake(),true);assert.equal(runtime.state,'idle');assert.equal(cleared,stale);stale.fn();assert.equal(runtime.state,'idle');runtime.dispose();});
